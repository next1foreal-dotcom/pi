...
export function createShellToolDefinition(
	...
	return {
		name: config.name,
		label: config.label,
		description: `Execute a ${config.shellName} command in the current working directory. Returns stdout and stderr. Output is truncated to last ${DEFAULT_MAX_LINES} lines or ${DEFAULT_MAX_BYTES / 1024}KB (whichever is hit first). If truncated, full output is saved to a temp file. Optionally provide a timeout in seconds; without one the command is killed after ${DEFAULT_TIMEOUT_SECONDS} seconds.`,
		promptSnippet: config.promptSnippet,
		promptGuidelines: [
			// Two 40-minute runs died on a recursive grep/find that walked node_modules; the
			// default timeout bounds the damage, this line prevents it. Shell-agnostic, so it
			// lives in the factory and is NOT gated on exposeSessionEnvironment.
