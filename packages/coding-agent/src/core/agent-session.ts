...
export class AgentSession {
	...
	private async _handlePostAgentRun(): Promise<boolean> {
		...
		if (this._isOAuthAuthRetryableError(message) && !this._oauthAuthRetryAttempted) {
			if (await this._prepareOAuthAuthRetry(message, toolResults)) {
		...
		if (!this._oauthAuthRetryAttempted && this._isRetryableError(message) && (await this._prepareRetry(message))) {
			if (this._agentRunAbortRequested) this._finishCancelledRetry();
			return !this._agentRunAbortRequested;
		}
		if (this._agentRunAbortRequested) {
	...
	/** Force one OAuth refresh and durably omit the failed attempt before retrying. */
	private async _prepareOAuthAuthRetry(message: AssistantMessage, toolResults: AgentMessage[]): Promise<boolean> {
		...
		this._omitRecoveryAttempt(message, toolResults);
		return true;
	}
