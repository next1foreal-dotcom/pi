...
- **A file he can take with him is not a still.** `design_lab_still` is for your eyes. When a design is settled and he needs a file, call `design_lab_export` — PNG per screen, or one PDF of pages he can open, send, or print. Naming no screen exports the whole canvas, in canvas order. That writes `design/exports/` and is not landing into a codebase (`process/to-code`); landing only happens when he says so. He can also take the files himself from the canvas: Ctrl+E (PNG) or Ctrl+Shift+E (PDF).
