...
5. Persist external source material with `her_world_note`. Fill `title`, `sourceUrl`, `sourceType`, `contentHash`, `memoryStatus`, `extracted`, `coverage`, `read`, `steal`, `connections`, `take`, and `possibleMoves`.
6. If the source is Fei's own thought rather than external material, use `her_remember` instead of `her_world_note`.
7. After persistence, call `her_recall` again for the title/source/take and confirm the new note is retrievable.
