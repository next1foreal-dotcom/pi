...
export async function evaluateTaskAcceptance(opts: {
	...
	if (plan?.gates.some(isEvidenceGate)) {
		...
		if (!selected) {
			evidenceFailureDetail = "result file is absent; checked raw log and jsonl log, but neither contained an evidence block with items";
			...
			const reason = selected.output.trim().length === 0 ? "is empty" : (parsed.error ?? "contains no evidence items");
