...
## 条件监视配方(Monitor 等价,2026-08-31 收录)

要等一个不归你管的条件(端口起了/文件出现/外部状态变了),别在回合里空转轮询——把"等"本身派成后台任务:

- objective: "watch: <条件一句话>"
- command: ["bash","-c","until <条件命令>; do sleep 15; done"]
- timeoutMinutes: <上限,兜底防永等>

条件满足→命令退出 0→.done 落盘→事件唤醒叫你;超时→failed 唤醒同样叫你(等不到也要知道)。这是 Claude harness 的 Monitor 原语在这里的形态:组合出来,不新造轮子。
